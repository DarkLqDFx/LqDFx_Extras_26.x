...
public class WaxWorkStation {

    @SubscribeEvent
    public static void onWorkstationRightClick(PlayerInteractEvent.RightClickBlock event) {
        ...
        boolean poiExists = poiManager.existsAtPosition(Objects.requireNonNull(poiKey), pos);
        ...
        if (held.getItem() instanceof HoneycombItem && poiExists) {
            ...
            player.sendSystemMessage(Component.translatable("lqdfxextras.display.wax_on").withStyle(ChatFormatting.GOLD));
        ...
        if (held.getItem() instanceof AxeItem) {
            if (PoiTypes.forState(state).isPresent() && !poiExists) {
                ...
                player.sendSystemMessage(Component.translatable("lqdfxextras.display.wax_off").withStyle(ChatFormatting.GREEN));
            }
            event.setCancellationResult(InteractionResult.SUCCESS);
            event.setCanceled(true);
        }
